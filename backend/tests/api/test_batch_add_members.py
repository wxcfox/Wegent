...
class TestBatchAddMembers:
    ...
    def test_batch_add_multiple_users_success(
        ...
        response = test_client.post(
            ...
            json={
                "members": [
                    {"user_id": u.id, "role": "Reporter"} for u in target_users
                ]
            },
    ...
    def test_batch_add_requires_manage_permission(
        ...
        """Non-owner without manage permission should be denied (404 because resource not visible)."""
        response = test_client.post(
            f"/api/share/KnowledgeBase/{knowledge_base.id}/members/batch",
            json={
                "members": [
        ...
        # Returns 404 because the KB is not visible to non-owner users without explicit access
